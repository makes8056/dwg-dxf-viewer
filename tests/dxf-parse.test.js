...
test('押し出しZが-1でも、楕円は左右反転させない（52章）', () => {
  // 【実物の図面で判明した不具合。ここを裏返すと図面が壊れます】
  //
  // DXFの決まりでは、**楕円の座標だけは最初から図面全体の座標（WCS）**で書かれている。
  // 円・円弧・文字は「その図形の面の中での座標（OCS）」なので裏返して読むが、
  // **楕円は裏返してはいけない。**
  //
  // お客様の図面には押し出しZ＝−1の楕円が644個あり、裏返していたせいで
  // 全部が左右反対の何もない場所へ飛んでいた（水色の点が散らばって見えた）。
  // 反転した先のまわりには図形が6個しか無く、戻した先には400個あった。
  const dxf = [
    '0', 'SECTION', '2', 'ENTITIES',
  ...
  assert.equal(e.cx, 10, '中心を裏返してしまっている（図面の反対側へ飛ぶ）');
  assert.equal(e.rotation, 0, '軸の傾きを裏返してしまっている');
  assert.equal(e.startAngle, 0, '始まりを裏返してしまっている');
  assert.equal(e.endAngle, 90, '終わりを裏返してしまっている');
});

test('押し出しZが-1でも、自由曲線と3次元の面を左右反転させない（52章）', () => {
  // 楕円と同じく、これらの座標も最初からWCSで書かれている
  const dxf = [
    '0', 'SECTION', '2', 'ENTITIES',
    '0', 'SPLINE', '8', '0', '62', '7', '230', '-1',
    '70', '8', '71', '1', '72', '0', '73', '2', '74', '0',
    '10', '100', '20', '0', '10', '200', '20', '50',
    '0', '3DFACE', '8', '0', '62', '7', '230', '-1', '70', '0',
    '10', '300', '20', '0', '11', '310', '21', '0',
    '12', '310', '22', '10', '13', '300', '23', '10',
    '0', 'ENDSEC', '0', 'EOF', '',
  ].join('\r\n');

  const d = parseDxf(dxf);
  const 曲線 = only(d, 'polyline')[0];
  assert.ok(曲線, '自由曲線が出ていない');
  near(曲線.points[0][0], 100, '自由曲線を裏返してしまっている');

  const 面 = only(d, 'line');
  assert.ok(面.length >= 3, '3次元の面が出ていない');
  assert.ok(
    面.every((l) => l.x1 >= 299 && l.x2 >= 299),
    '3次元の面を裏返してしまっている（Xがマイナスになっている）'
  );
});
