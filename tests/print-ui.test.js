...
test('ボタンは7つだけ（図面を開く／図面を選ぶ／全体表示／長さを測る／文字を書く／白黒で表示／印刷する範囲）', () => {
  // ボタンをむやみに増やさない（開発ルール11章）。
  // 「拡大」「縮小」はユーザー判断で外した。2本指のつまむ操作でできるため。
  // 「文字を書く」は v0.3.8 で足した（開発ルール43章）。
  // 「白黒で表示」は v0.4.9 で足した（開発ルール53章。ユーザーの指示）。
...
// ============================================================
// 白黒で表示（v0.4.9／開発ルール53章）
// ============================================================

test('「白黒で表示」は、入っているか切れているかが分かるボタンになっている', () => {
  // 現場では手袋ごしに一目で分かる必要がある。
  // 押しても見た目が変わらないと、今どちらなのか分からない。
  const js = read('src/ui/toolbar.js');
  assert.match(js, /aria-pressed/, '入り切りの状態を持っていない');
  const css = read('src/ui/ui.css');
  assert.match(css, /aria-pressed=['"]true['"]/, '入っているときの見た目が決めてない');
});

test('白黒の指定を、画面・確認の絵・PDFの3つすべてに渡している', () => {
  // 【これがこのアプリで一番くり返している事故】
  // 図面を描く道は3本ある。1本でも渡し忘れると、
  // **画面では黒いのに紙だけ赤い**（またはその逆）になる。
  // 紙に出すまで気づけないので、ここで数えて守る（開発ルール36.2）。
  const app = read('src/ui/app.js');
  assert.match(app, /renderDrawing\([\s\S]*?monochrome:/, '画面に渡していない');
  assert.match(app, /createPrintImage\([^)]*monochrome/, '確認の絵に渡していない');
  assert.match(app, /createPrintPdf\([^)]*monochrome/, 'PDFに渡していない');
});

test('白黒にしたことを覚えておく（開き直しても押し直さなくてよい）', () => {
  const app = read('src/ui/app.js');
  assert.match(app, /localStorage[\s\S]*monochrome|monochrome[\s\S]{0,400}localStorage/i,
    '覚えていない（開くたびに押し直しになる）');
});
