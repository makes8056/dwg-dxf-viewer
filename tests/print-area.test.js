...
//
// 【ここを忘れると起きること】
// 図面を描く道は3本ある（画面・確認の絵・PDF）。
// 確認の絵に渡し忘れると、**画面もPDFも黒いのに、確認画面だけ色が付く。**
// 押す直前に見るのが確認画面なので、これが一番まぎらわしい。
// ============================================================

/** 線を描いたときの色を、順番に記録するCanvasのふり。 */
function 色を記録するCanvas(width, height) {
...
const 赤い線の図面 = {
  ...
  contentBounds: { minX: 0, minY: 0, maxX: 100, maxY: 60 },
  layers: [],
  entities: [{ type: 'line', layer: 'PIPE', color: '#ff0000', x1: 0, y1: 0, x2: 100, y2: 60 }],
  unsupported: { count: 0, kinds: {} },
  source: 'dxf',
};

test('確認の絵も、白黒にすれば黒で描かれる', () => {
  const area = { minX: 0, minY: 0, maxX: 100, maxY: 60 };
  let 作った = null;
  renderPrintCanvas(赤い線の図面, area, {
    createCanvas: (w, h) => { 作った = 色を記録するCanvas(w, h); return 作った; },
    monochrome: true,
  });
  assert.ok(作った.使った色.length > 0, '線が描かれていない');
  assert.ok(作った.使った色.every((c) => c === '#000000'), `黒でない色がある: ${作った.使った色.join()}`);
});

test('確認の絵は、白黒にしなければ赤いまま', () => {
  const area = { minX: 0, minY: 0, maxX: 100, maxY: 60 };
  let 作った = null;
  renderPrintCanvas(赤い線の図面, area, {
    createCanvas: (w, h) => { 作った = 色を記録するCanvas(w, h); return 作った; },
  });
  assert.ok(作った.使った色.includes('#ff0000'), `赤が入っていない: ${作った.使った色.join()}`);
});
