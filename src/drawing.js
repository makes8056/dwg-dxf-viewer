...
/**
 * 「大文字の高さ」が、文字枠ぜんたいの何割か（開発ルール48章）。
 *
 * 【ここを忘れると、文字がぜんぶ小さく・下寄りに出ます】
 *
 * CADの「文字の高さ」は、**大文字（A・8など）そのものの高さ**である。
 * ところが CanvasやPDFに渡す数値は、**文字枠ぜんたい（em）の大きさ**で、
 * その中で大文字が占めるのは7割ほどしかない。
 *
 * そのまま渡すと、
 *   - 文字が指定より**3割ちかく小さく**出る
 *   - 書き出しの線（ベースライン）は合っているので、**下に寄って**見える
 * ということが起きる。実物の図面で、俵型マークの中の記号が中心から外れ、
 * 図面番号の文字が小さく出ていた。
 *
 * そこで、渡すときに **この割合で割って**、大文字の高さが指定どおりになるようにする。
 *
 * 【なぜ実測せずに決め打ちか】
 *   画面（Canvas）と紙（PDF）で別々に測ると、わずかでも食い違ったときに
 *   **確認画面では合っているのに紙だけずれる**（開発ルール36.2）。
 *   画面もPDFも Helvetica系の書体を使い、その大文字の高さはどちらも0.72前後なので、
 *   1つの決まった数を両方で使うほうが安全である。
 */
...
/**
 * この図形を、実際に何色で描くかを決める（開発ルール53章）。
 *
 * 【なぜ、この決め方を1か所にまとめるのか（開発ルール36.2）】
 *   図面を描く道は3本ある。**画面・印刷用の絵（PNG）・PDF** である。
 *   色の決め方をそれぞれに書くと、どれか1本を直し忘れたときに
 *   **画面では黒いのに紙だけ赤い**という食い違いが起きる。
 *   3本ともこの関数だけを通すことで、食い違いようがなくする。
 *
 * 【なぜ、書き足した文字（注記）は白黒にしないのか】
 *   注記の色は、ユーザーが5色から**わざわざ選んで付けた印**である
 *   （開発ルール44.2）。白黒にすると、その使い分けが消えるうえに、
 *   図面のもとの線と見分けが付かなくなる。
 *   注記の5色は白黒プリンターでも読み分けられることを実機で確認済みなので
 *   （2026-09-08）、黒くする必要もない。
 *
 * @param {object} entity 図形
 * @param {boolean} [monochrome] 白黒で表示するか
 * @returns {string} CSSの色文字列
 */
export function entityColor(entity, monochrome) {
  if (monochrome && !(entity && entity.isNote)) return MONOCHROME_COLOR;
  return (entity && entity.color) || MONOCHROME_COLOR;
}
